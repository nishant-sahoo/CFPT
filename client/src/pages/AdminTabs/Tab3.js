...
function Tab3() {
    ...
    async function fetchData() {
      ...
      if (Object.keys(selectValue).length !== 0) {
        try {
          ...
          const problemSets = data.map((response) => response.data);
  ...
  const handleSubmit = async (e) => {
    ...
    if (e.nativeEvent.submitter.name === "addPbtn") {
      ...
      try {
        const res = await client.delete(`create/add/${deleteProb.cvalue._id}`);
